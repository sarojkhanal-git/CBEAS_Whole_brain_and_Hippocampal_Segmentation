...
class SwinUNETR(nn.Module):
    ...
    def __init__(
        ...
        self.adapters10 = ConvAdapter(channels=16*feature_size)
        self.adapters5 = ConvAdapter(channels=8*feature_size)
        self.adapters4 = ConvAdapter(channels=4*feature_size)
        self.adapters3 = ConvAdapter(channels=2*feature_size)
        self.adapters2 = ConvAdapter(channels=feature_size)
        self.adapters1 = ConvAdapter(channels=feature_size)
    ...
    def forward(self, x_in):
        ...
        dec4 = self.adapters10(dec4)        
        dec3 = self.decoder5(dec4, hidden_states_out[3])
        dec3 = self.adapters5(dec3)
        dec2 = self.decoder4(dec3, enc3)
        dec2 = self.adapters4(dec2)
        dec1 = self.decoder3(dec2, enc2)
        dec1 = self.adapters3(dec1)
        dec0 = self.decoder2(dec1, enc1)
        dec0 = self.adapters2(dec0)
        out = self.decoder1(dec0, enc0)
        out = self.adapters1(out)
        logits = self.out(out)
        return logits
...
class ConvAdapter(nn.Module):
    def __init__ (self, channels: int, act_layer=nn.ReLU, perm=False):
        super().__init__()
    ...
    def forward(self, x):
        if self.perm:
            x = x.permute(0, 4, 1, 2, 3) #was expecting (b, d, h, w, c) so changed to (b, c, d, h, w)
        ...
        out = x+out
        if self.perm:
            out = out.permute(0, 2, 3, 4, 1) #changed back to (b, d, h, w, c)
...
class BasicLayer(nn.Module):
    ...
    def __init__(
        ...
        # -------ADAPTER ADDITION----------------
        self.adapters = ConvAdapter(channels=dim, perm=True)
        #----------------------------------------
    ...
    def forward(self, x):
        ...
        if len(x_shape) == 5:
            ...
            #----------------ADAPTER AFTER SWIN TRANSFORMER------------  
            x = self.adapters(x)
            #-------------------------------------
